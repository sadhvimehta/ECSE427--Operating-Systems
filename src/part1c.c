...
int my_system(char* command) {
	...
	int childPid = vfork(); // stores return value of vfork in respective processes
	...
	if(childPid < 0) {
		...
		// call _exit() and not exit() b/c parent and child sharing address space
		// _exit() prevents closing of open I/O streams used by parent process and maintains memory
		_exit(127);
	}
